...
import (
	...
	"wiki/app/models"
	post "wiki/app/repository"
)

// NewSQLPostRepo retunrs implement of post repository interface
func NewSQLPostRepo(Conn *sql.DB) post.PostRepo {
...
func (m *mysqlPostRepo) Create(ctx context.Context, p *models.Post)(int64, error) {
	...
	statement, err := m.Conn.PrepareContext(ctx, query)

	if err != nil {
		return -1, err
	}

	result, err := statement.ExecContext(ctx, p.Title, p.Content)
